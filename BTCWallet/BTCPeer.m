...
@property (nonatomic, assign) id<BTCPeerDelegate> delegate;
...
@property (nonatomic, strong) BTCMerkleBlock *currentBlock;
...
- (void)connect
{
    if (self.status != BTCPeerStatusDisconnected) return;
    _status = BTCPeerStatusConnecting;
    _pingTime = DBL_MAX;
                ...
                self.reachabilityObserver =
                    ...
                    queue:nil usingBlock:^(NSNotification *note) {
                        if (self.reachabilityObserver && self.reachability.currentReachabilityStatus != NotReachable) {
                            _status = BTCPeerStatusDisconnected;
                            [self connect];
                        }
                    }];
    ...
    dispatch_async(dispatch_queue_create(label.UTF8String, NULL), ^{
        ...
        [self performSelector:@selector(disconnectWithError:)
         withObject:[NSError errorWithDomain:@"BTCWallet" code:BITCOIN_TIMEOUT_CODE
...
- (void)error:(NSString *)message, ... NS_FORMAT_FUNCTION(1,2)
{
    ...
    [self disconnectWithError:[NSError errorWithDomain:@"BTCWallet" code:500
...
- (void)didConnect
{
    if (self.status != BTCPeerStatusConnecting || ! self.sentVerack || ! self.gotVerack) return;
    ...
    _status = BTCPeerStatusConnected;

    dispatch_async(self.delegateQueue, ^{
        if (_status == BTCPeerStatusConnected) [self.delegate peerConnected:self];
    });
}
...
- (void)acceptAddrMessage:(NSData *)message
{
    ...
    for (NSUInteger off = l; off < l + 30*count; off += 30) {
        ...
        [peers addObject:[[BTCPeer alloc] initWithAddress:address port:port timestamp:timestamp - 2*60*60
         services:services]];
    }

    dispatch_async(self.delegateQueue, ^{
        if (_status == BTCPeerStatusConnected) [self.delegate peer:self relayedPeers:peers];
    });
}
...
- (void)acceptInvMessage:(NSData *)message
{
    ...
    if ([txHashes intersectsOrderedSet:self.knownTxHashes]) { // remove transactions we already have
        for (NSValue *hash in txHashes) {
            ...
            dispatch_async(self.delegateQueue, ^{
                if (_status == BTCPeerStatusConnected) [self.delegate peer:self hasTransaction:h];
            });
        }
...
{
    BTCTransaction *tx = [BTCTransaction transactionWithMessage:message];
    ...
    if (self.currentBlock) { // we're collecting tx messages for a merkleblock
        ...
        if (self.currentBlockTxHashes.count == 0) { // we received the entire block including all matched tx
            BTCMerkleBlock *block = self.currentBlock;
...
- (void)acceptHeadersMessage:(NSData *)message
{
    ...
    for (NSUInteger off = l; off < l + 81*count; off += 81) {
        BTCMerkleBlock *block = [BTCMerkleBlock blockWithMessage:[message subdataWithRange:NSMakeRange(off, 81)]];
...
- (void)acceptGetdataMessage:(NSData *)message
{
    ...
    dispatch_async(self.delegateQueue, ^{
        ...
        for (NSUInteger off = l; off < l + count*36; off += 36) {
            inv_type type = [message UInt32AtOffset:off];
            UInt256 hash = [message hashAtOffset:off + sizeof(uint32_t)];
            BTCTransaction *transaction = nil;
...
- (void)acceptPongMessage:(NSData *)message
{
    ...
    dispatch_async(self.delegateQueue, ^{
        if (_status == BTCPeerStatusConnected && self.pongHandlers.count) {
...
- (void)acceptMerkleblockMessage:(NSData *)message
{
    ...
    BTCMerkleBlock *block = [BTCMerkleBlock blockWithMessage:message];
...
{
    return (self == object || ([object isKindOfClass:[BTCPeer class]] && _port == ((BTCPeer *)object).port &&
                               uint128_eq(_address, [(BTCPeer *)object address]))) ? YES : NO;
}
