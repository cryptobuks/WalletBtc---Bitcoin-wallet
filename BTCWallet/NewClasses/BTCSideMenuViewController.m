...
typedef NS_ENUM(NSInteger, BTCSideMenuAction){
    BTCSideMenuOpen,
    BTCSideMenuClose
};

typedef struct {
    BTCSideMenuAction menuAction;
    BOOL shouldBounce;
    CGFloat velocity;
} BTCSideMenuPanResultInfo;

@interface BTCSideMenuViewController () <UIGestureRecognizerDelegate>
...
@implementation BTCSideMenuViewController

- (id)initWithCoder:(NSCoder *)aDecoder {
    
    self = [super initWithCoder:aDecoder];
    if (self) {
        _options = [[BTCSideMenuOptions alloc] init];
    }
...
{
    return [self initWithMenuViewController:menuViewController contentViewController:contentViewController options:[[BTCSideMenuOptions alloc] init]];
}

- (instancetype)initWithMenuViewController:(UIViewController *)menuViewController
                     contentViewController:(UIViewController *)contentViewController
                                   options:(BTCSideMenuOptions *)options
{
    self = [self init];
    if (self) {
        _options = options;
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)panGesture {
    ...
    switch (panGesture.state) {
        ...
        case UIGestureRecognizerStateEnded:{
            ...
            BTCSideMenuPanResultInfo panInfo = [self panResultInfoForVelocity:velocity];
            
            if (panInfo.menuAction == BTCSideMenuOpen) {
...
- (BTCSideMenuPanResultInfo)panResultInfoForVelocity:(CGPoint)velocity {
    ...
    BTCSideMenuPanResultInfo panInfo = {BTCSideMenuClose, NO, 0.0f};
    
    panInfo.menuAction = menuOrigin <= pointOfNoReturn ? BTCSideMenuClose : BTCSideMenuOpen;
    
    if (velocity.x >= thresholdVelocity) {
        panInfo.menuAction = BTCSideMenuOpen;
        panInfo.velocity = velocity.x;
    } else if (velocity.x <= (-1.0f * thresholdVelocity)) {
        panInfo.menuAction = BTCSideMenuClose;
        panInfo.velocity = velocity.x;
    }
...
@implementation UIViewController (BTCSideMenuViewController)
...
- (BTCSideMenuViewController *)sideMenuController {
    ...
    while (viewController) {
        if ([viewController isKindOfClass:[BTCSideMenuViewController class]])
            return (BTCSideMenuViewController *)viewController;
...
- (void)addLeftMenuButtonWithImage:(UIImage *)buttonImage {
    ...
    menuButton.tintColor = [UIColor colorWithRed:241.0/255.0 green:149.0/255.0 blue:81.0/255.0 alpha:1.0];
