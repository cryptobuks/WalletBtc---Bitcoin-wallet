...
static NSUInteger txAddressIndex(BTCTransaction *tx, NSArray *chain) {
...
@property (nonatomic, strong) id<BTCKeySequence> sequence;
...
@implementation BTCWallet

- (instancetype)initWithContext:(NSManagedObjectContext *)context sequence:(id<BTCKeySequence>)sequence
masterPublicKey:(NSData *)masterPublicKey seed:(NSData *(^)(NSString *authprompt, uint64_t amount))seed
{
    ...
    [self.moc performBlockAndWait:^{
        [BTCAddressEntity setContext:self.moc];
        [BTCTransactionEntity setContext:self.moc];
        [BTCTxMetadataEntity setContext:self.moc];

        for (BTCAddressEntity *e in [BTCAddressEntity allObjects]) {
            @autoreleasepool {
        ...
        for (BTCTxMetadataEntity *e in [BTCTxMetadataEntity allObjects]) {
            @autoreleasepool {
                if (e.type != TX_MDTYPE_MSG) continue;
                
                BTCTransaction *tx = e.transaction;
        ...
        if ([BTCTransactionEntity countAllObjects] > self.allTx.count) {
            // pre-fetch transaction inputs and outputs
            [BTCTxInputEntity allObjects];
            [BTCTxOutputEntity allObjects];
            
            for (BTCTransactionEntity *e in [BTCTransactionEntity allObjects]) {
                @autoreleasepool {
                    BTCTransaction *tx = e.transaction;
                    NSValue *hash = (tx) ? uint256_obj(tx.txHash) : nil;
    ...
    if (updateTx.count > 0) {
        [self.moc performBlock:^{
            for (BTCTransaction *tx in updateTx) {
                [[BTCTxMetadataEntity managedObject] setAttributesFromTx:tx];
            }
            
            [BTCTxMetadataEntity saveContext];
        }];
    }
...
- (NSArray *)addressesWithGapLimit:(NSUInteger)gapLimit internal:(BOOL)internal
{
    ...
    @synchronized(self) {
        ...
        while (a.count < gapLimit) { // generate new addresses up to gapLimit
            ...
            NSString *addr = [BTCKey keyWithPublicKey:pubKey].address;
...
- (void)sortTransactions
{
    ...
    __block __weak BOOL (^_isAscending)(id, id) = isAscending = ^BOOL(BTCTransaction *tx1, BTCTransaction *tx2) {
...
- (void)updateBalance
{
    ...
    for (BTCTransaction *tx in [self.transactions reverseObjectEnumerator]) {
        @autoreleasepool {
            ...
            for (NSValue *hash in tx.inputHashes) {
                ...
                [hash getValue:&h];
                [spent addObject:brutxo_obj(((BTCUTXO) { h, n }))];
            }
            ...
            for (NSString *address in tx.outputAddresses) { // add outputs to UTXO set
                if ([self containsAddress:address]) {
                    [utxos addObject:brutxo_obj(((BTCUTXO) { tx.txHash, n }))];
...
{
    [[NSNotificationCenter defaultCenter] postNotificationName:BTCWalletBalanceChangedNotification object:nil];
}
...
- (BTCTransaction *)transactionFor:(uint64_t)amount to:(NSString *)address withFee:(BOOL)fee
{
...
- (BTCTransaction *)transactionForAmounts:(NSArray *)amounts toOutputScripts:(NSArray *)scripts withFee:(BOOL)fee
{
    uint64_t amount = 0, balance = 0, feeAmount = 0;
    BTCTransaction *transaction = [BTCTransaction new], *tx;
...
- (BOOL)signTransaction:(BTCTransaction *)transaction withPrompt:(NSString *)authprompt
{
...
- (BOOL)containsTransaction:(BTCTransaction *)transaction
{
    ...
    for (NSValue *txHash in transaction.inputHashes) {
        BTCTransaction *tx = self.allTx[txHash];
...
- (BOOL)registerTransaction:(BTCTransaction *)transaction
{
    ...
    NSLog(@"[BTCWallet] received unseen transaction %@", transaction);
    ...
    [self.moc performBlock:^{ // add the transaction to core data
        if ([BTCTransactionEntity countObjectsMatching:@"txHash == %@",
             [NSData dataWithBytes:&txHash length:sizeof(txHash)]] == 0) {
            [[BTCTransactionEntity managedObject] setAttributesFromTx:transaction];
        }
        
        if ([BTCTxMetadataEntity countObjectsMatching:@"txHash == %@ && type == %d",
             [NSData dataWithBytes:&txHash length:sizeof(txHash)], TX_MDTYPE_MSG] == 0) {
            [[BTCTxMetadataEntity managedObject] setAttributesFromTx:transaction];
        }
    }];
...
{
    BTCTransaction *transaction = self.allTx[uint256_obj(txHash)];
    NSMutableSet *hashes = [NSMutableSet set];

    for (BTCTransaction *tx in self.transactions) { // remove dependent transactions
    ...
    [self.moc performBlock:^{ // remove transaction from core data
        [BTCTransactionEntity deleteObjects:[BTCTransactionEntity objectsMatching:@"txHash == %@",
                                            [NSData dataWithBytes:&txHash length:sizeof(txHash)]]];
        [BTCTxMetadataEntity deleteObjects:[BTCTxMetadataEntity objectsMatching:@"txHash == %@",
                                           [NSData dataWithBytes:&txHash length:sizeof(txHash)]]];
    }];
}

// returns the transaction with the given hash if it's been registered in the wallet (might also return non-registered)
- (BTCTransaction *)transactionForHash:(UInt256)txHash
{
...
- (BOOL)transactionIsValid:(BTCTransaction *)transaction
{
    ...
    for (NSValue *hash in transaction.inputHashes) {
        BTCTransaction *tx = self.allTx[hash];
        uint32_t n = [transaction.inputIndexes[i++] unsignedIntValue];
        UInt256 h;

        [hash getValue:&h];
        if ((tx && ! [self transactionIsValid:tx]) ||
            [self.spentOutputs containsObject:brutxo_obj(((BTCUTXO) { h, n }))]) return NO;
    }
...
- (BOOL)transactionIsPending:(BTCTransaction *)transaction
{
...
- (BOOL)transactionIsVerified:(BTCTransaction *)transaction
{
    ...
    if (hashes.count > 0) {
        ...
        [self.moc performBlockAndWait:^{
            @autoreleasepool {
                ...
                for (BTCTransactionEntity *e in [BTCTransactionEntity objectsMatching:@"txHash in %@", hashes]) {
                    e.blockHeight = height;
                    e.timestamp = timestamp;
                    [entities addObject:e];
                }
            
                for (BTCTxMetadataEntity *e in [BTCTxMetadataEntity objectsMatching:@"txHash in %@ && type == %d", hashes,
                                               TX_MDTYPE_MSG]) {
                    @autoreleasepool {
                        BTCTransaction *tx = e.transaction;
...
- (uint64_t)amountReceivedFromTransaction:(BTCTransaction *)transaction
{
    uint64_t amount = 0;
    NSUInteger n = 0;
...
- (uint64_t)amountSentByTransaction:(BTCTransaction *)transaction
{
    uint64_t amount = 0;
    NSUInteger i = 0;
...
- (uint64_t)feeForTransaction:(BTCTransaction *)transaction
{
    uint64_t amount = 0;
    NSUInteger i = 0;
...
- (uint64_t)balanceAfterTransaction:(BTCTransaction *)transaction
{
...
- (uint32_t)blockHeightUntilFree:(BTCTransaction *)transaction
{
...
- (uint64_t)feeForTxSize:(NSUInteger)size
{
    ...
    fee = (self.feePerKb / 1024) * size;
  //  fee = fee / 2.0;
    NSLog(@"+++++++  fee = %lld  stdfee = %lld size = %lu", fee, standardFee, (unsigned long)size);
