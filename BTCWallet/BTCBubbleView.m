...
{
    BTCBubbleView *v = [[self alloc] initWithFrame:CGRectMake(center.x - MARGIN_X, center.y - MARGIN_Y, MARGIN_X*2,
                                                             MARGIN_Y*2)];
...
{
    BTCBubbleView *v = [[self alloc] initWithFrame:CGRectMake(0, 0, MARGIN_X*2, MARGIN_Y*2)];
...
- (void)setTipDirection:(BTCBubbleTipDirection)tipDirection
{
...
- (void)layoutSubviews
{
    ...
    if (self.tipPoint.x > 1) { // position bubble to point to tipPoint
        ...
        center.y += (self.tipDirection == BTCBubbleTipDirectionUp ? 1 : -1)*((rect.size.height + MARGIN_Y*2)/2 + RADIUS);
    }
    ...
    if (self.tipPoint.x > 1) { // draw tip arrow
        ...
        if (self.tipDirection == BTCBubbleTipDirectionUp) {
