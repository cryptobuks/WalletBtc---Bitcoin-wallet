...
{
    uint8_t buf[sizeof(BTCECPoint) + sizeof(i)];
    UInt512 I;
    ...
    else BTCSecp256k1PointGen((BTCECPoint *)buf, k);

    *(uint32_t *)&buf[sizeof(BTCECPoint)] = CFSwapInt32HostToBig(i);

    HMAC(&I, SHA512, sizeof(UInt512), c, sizeof(*c), buf, sizeof(buf)); // I = HMAC-SHA512(c, k|P(k) || i)
    
    BTCSecp256k1ModAdd(k, (UInt256 *)&I); // k = IL + k (mod n)
...
//
static void CKDpub(BTCECPoint *K, UInt256 *c, uint32_t i)
{
    ...
    *c = *(UInt256 *)&I.u8[sizeof(UInt256)]; // c = IR
    BTCSecp256k1PointAdd(K, (UInt256 *)&I); // K = P(IL) + K
...
@implementation BTCBIP32Sequence

#pragma mark - BTCKeySequence
...
- (NSData *)masterPublicKeyFromSeed:(NSData *)seed
{
    ...
    [mpk appendBytes:[BTCKey keyWithSecret:secret compressed:YES].hash160.u32 length:4];
    
    CKDpriv(&secret, &chain, 0 | BIP32_HARD); // account 0H

    [mpk appendBytes:&chain length:sizeof(chain)];
    [mpk appendData:[BTCKey keyWithSecret:secret compressed:YES].publicKey];
...
{
    if (masterPublicKey.length < 4 + sizeof(UInt256) + sizeof(BTCECPoint)) return nil;

    UInt256 chain = *(const UInt256 *)((const uint8_t *)masterPublicKey.bytes + 4);
    BTCECPoint pubKey = *(const BTCECPoint *)((const uint8_t *)masterPublicKey.bytes + 36);
...
- (NSString *)serializedMasterPublicKey:(NSData *)masterPublicKey
{
    ...
    BTCECPoint pubKey = *(BTCECPoint *)((const uint8_t *)masterPublicKey.bytes + 36);
