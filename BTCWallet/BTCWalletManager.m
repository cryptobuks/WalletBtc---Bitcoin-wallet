...
#define FEE_PER_KB_URL       @"https://bitcoinfees.21.co/api/v1/fees/recommended"
#define TICKER_FAILOVER_URL  @"https://bitpay.com/rates"

#define USER_AGENT [NSString stringWithFormat:@"/wallet:%@/",\
                    NSBundle.mainBundle.infoDictionary[@"CFBundleShortVersionString"]]

#define SEED_ENTROPY_LENGTH   (128/8)
#define SEC_ATTR_SERVICE      @"xxx.trubs"
...
#define USER_ACCOUNT_KEY    @"keyfortheuseraccount"
...
@interface BTCWalletManager()

@property (nonatomic, strong) BTCWallet *wallet;
...
@property (nonatomic, strong) void (^sweepCompletion)(BTCTransaction *tx, uint64_t fee, NSError *error);
...
@implementation BTCWalletManager
...
- (instancetype)init
{
    ...
    self.sequence = [BTCBIP32Sequence new];
    self.mnemonic = [BTCBIP39Mnemonic new];
...
- (BTCWallet *)wallet
{
    ...
    @synchronized(self) {
        ...
        _wallet =
            [[BTCWallet alloc] initWithContext:[NSManagedObject context] sequence:self.sequence
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            BTCKey *k = [BTCKey keyWithPublicKey:[self.sequence publicKey:0 internal:NO masterPublicKey:mpk]];
            ...
            if (_wallet.allReceiveAddresses.count > 0 && k && ! [_wallet containsAddress:k.address]) {
                ...
                [[NSManagedObject context] performBlockAndWait:^{
                    [BTCAddressEntity deleteObjects:[BTCAddressEntity allObjects]];
                    [BTCTransactionEntity deleteObjects:[BTCTransactionEntity allObjects]];
                    [BTCTxMetadataEntity deleteObjects:[BTCTxMetadataEntity allObjects]];
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [[NSNotificationCenter defaultCenter] postNotificationName:BTCWalletManagerSeedChangedNotification
                     object:nil];
                    [[NSNotificationCenter defaultCenter] postNotificationName:BTCWalletBalanceChangedNotification
                     object:nil];
                });
            }
        });
...
- (void)setSeedPhrase:(NSString *)seedPhrase
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [[NSNotificationCenter defaultCenter] postNotificationName:BTCWalletManagerSeedChangedNotification object:nil];
...
- (NSString *)authPrivateKey
{
    @autoreleasepool {
        ...
        if (! privKey) {
            ...
            privKey = [[BTCBIP32Sequence new] authPrivateKeyFromSeed:seed];
...
- (NSString*)pin{
    return getKeychainString(PIN_KEY, nil);
}
  ...
  //  [req setValue:USER_AGENT forHTTPHeaderField:@"User-Agent"];
    ...
    completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if (error || ! [json isKindOfClass:[NSDictionary class]] ||
            ! [json[@"fastestFee"] isKindOfClass:[NSNumber class]]) {
            NSLog(@"unexpected response from %@:\n%@", req.URL.host,
                  [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]);
            return;
        }
      ...
      //  uint64_t newFee = [json[@"fee_per_kb"] unsignedLongLongValue];
        uint64_t newFee;
        switch ([[NSUserDefaults standardUserDefaults] integerForKey:@"trzPriority"]) {
            case 0:
                newFee = [json[@"fastestFee"] unsignedLongLongValue] * 1024;
                break;
            case 1:
                newFee = [json[@"halfHourFee"] unsignedLongLongValue] * 1024;
                newFee = newFee / 1.5;
                break;
            case 2:
                newFee = [json[@"hourFee"] unsignedLongLongValue] * 1024;
                newFee = newFee / 2.0;
                break;
...
// queries and calls the completion block with unspent outputs for the given addresses
...
completion:(void (^)(NSArray *utxos, NSArray *amounts, NSArray *scripts, NSError *error))completion
{
    [self utxos:UNSPENT_FAILOVER_URL forAddresses:addresses
     completion:^(NSArray *utxos, NSArray *amounts, NSArray *scripts, NSError *error) {
         completion(utxos, amounts, scripts, error);
     }];
}
    ...
    completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if (! [json isKindOfClass:[NSArray class]]) {
            completion(nil, nil, nil,
                       [NSError errorWithDomain:@"BTCWallet" code:417 userInfo:@{NSLocalizedDescriptionKey:
...
// given a private key, queries for unspent outputs and calls the completion block with a signed transaction
// that will sweep the balance into the wallet (doesn't publish the tx)
- (void)sweepPrivateKey:(NSString *)privKey withFee:(BOOL)fee
completion:(void (^)(BTCTransaction *tx, uint64_t fee, NSError *error))completion
{
    ...
    BTCKey *key = [BTCKey keyWithPrivateKey:privKey];

    if (! key.address) {
        completion(nil, 0, [NSError errorWithDomain:@"BTCWallet" code:187 userInfo:@{NSLocalizedDescriptionKey:
    ...
    completion:^(NSArray *utxos, NSArray *amounts, NSArray *scripts, NSError *error) {
        ...
        if (balance == 0) {
            completion(nil, 0, [NSError errorWithDomain:@"BTCWallet" code:417 userInfo:@{NSLocalizedDescriptionKey:
        ...
        if (! [tx signWithPrivateKeys:@[privKey]]) {
            completion(nil, 0, [NSError errorWithDomain:@"BTCWallet" code:401 userInfo:@{NSLocalizedDescriptionKey:
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    if (buttonIndex == alertView.cancelButtonIndex) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            BTCKey *key = [BTCKey keyWithBIP38Key:self.sweepKey andPassphrase:passphrase];
