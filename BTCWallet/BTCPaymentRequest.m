...
- (BTCPaymentProtocolRequest *)protocolRequest
{
    ...
    BTCPaymentProtocolDetails *details =
        [[BTCPaymentProtocolDetails alloc] initWithNetwork:network outputAmounts:@[@(self.amount)]
         outputScripts:@[script] time:0 expires:0 memo:self.message paymentURL:nil merchantData:nil];
    BTCPaymentProtocolRequest *request =
        [[BTCPaymentProtocolRequest alloc] initWithVersion:1 pkiType:@"none" certs:(name ? @[name] : nil) details:details
         signature:nil];
...
//  [req addValue:@"text/uri-list" forHTTPHeaderField:@"Accept"]; // breaks some BIP72 implementations, notably bitpay's
    ...
    if (! req) {
        completion(nil, [NSError errorWithDomain:@"BTCWallet" code:417
    ...
    completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        BTCPaymentProtocolRequest *request = nil;
        ...
        if ([response.MIMEType.lowercaseString isEqual:@"application/bitcoin-paymentrequest"] && data.length <= 50000) {
            request = [BTCPaymentProtocolRequest requestWithData:data];
        }
            ...
            for (NSString *url in [[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]
                ...
                request = [BTCPaymentRequest requestWithString:url].protocolRequest; // use first url and ignore the rest
                break;
            }
        }
        ...
        if (! request) {
            ...
            completion(nil, [NSError errorWithDomain:@"BTCWallet" code:417 userInfo:@{NSLocalizedDescriptionKey:
...
+ (void)postPayment:(BTCPaymentProtocolPayment *)payment to:(NSString *)paymentURL timeout:(NSTimeInterval)timeout
completion:(void (^)(BTCPaymentProtocolACK *ack, NSError *error))completion
{
