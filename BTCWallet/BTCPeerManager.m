...
static const char *dns_seeds[] = {
    "testnet-seed.btcwallet.com.", "testnet-seed.bitcoin.petertodd.org.", "testnet-seed.bluematt.me.",
...
static const char *dns_seeds[] = {
    "seed.btcwallet.com.", "seed.bitcoin.sipa.be.", "dnsseed.bluematt.me.", "dnsseed.bitcoin.dashjr.org.",
    "seed.bitcoinstats.com.", "bitseed.xf2.org.", "seed.bitcoin.jonasschnelli.ch."
};

#endif

@interface BTCPeerManager ()
...
@property (nonatomic, strong) BTCPeer *downloadPeer;
@property (nonatomic, assign) uint32_t syncStartHeight, filterUpdateHeight;
@property (nonatomic, strong) BTCBloomFilter *bloomFilter;
...
@property (nonatomic, strong) BTCMerkleBlock *lastBlock, *lastOrphan;
@property (nonatomic, strong) dispatch_queue_t q;
@property (nonatomic, strong) id backgroundObserver, seedObserver;

@end

@implementation BTCPeerManager
...
- (instancetype)init
{
    ...
    self.earliestKeyTime = [BTCWalletManager sharedInstance].seedCreationTime;
    ...
    self.seedObserver =
        ...
        queue:nil usingBlock:^(NSNotification *note) {
            ...
            [BTCMerkleBlockEntity deleteObjects:[BTCMerkleBlockEntity allObjects]];
            [BTCMerkleBlockEntity saveContext];
            _blocks = nil;
            _bloomFilter = nil;
            _lastBlock = nil;
...
- (NSMutableOrderedSet *)peers
{
    ...
    @synchronized(self) {
        ...
        [[BTCPeerEntity context] performBlockAndWait:^{
            for (BTCPeerEntity *e in [BTCPeerEntity allObjects]) {
                @autoreleasepool {
        ...
        if (_peers.count < PEER_MAX_CONNECTIONS ||
            ((BTCPeer *)_peers[PEER_MAX_CONNECTIONS - 1]).timestamp + 3*24*60*60 < now) {
...
//                        }
                        ...
                        [peers[i] addObject:[[BTCPeer alloc] initWithAddress:addr port:port
            ...
            if (_peers.count < PEER_MAX_CONNECTIONS) {
                ...
                for (NSNumber *address in [NSArray arrayWithContentsOfFile:[[NSBundle mainBundle]
                    ...
                    [_peers addObject:[[BTCPeer alloc] initWithAddress:addr port:BITCOIN_STANDARD_PORT
...
- (NSMutableDictionary *)blocks
{
    ...
    [[BTCMerkleBlockEntity context] performBlockAndWait:^{
        ...
        for (int i = 0; i < CHECKPOINT_COUNT; i++) { // add checkpoints to the block collection
            ...
            _blocks[uint256_obj(hash)] = [[BTCMerkleBlock alloc] initWithBlockHash:hash version:1 prevBlock:UINT256_ZERO
        ...
        for (BTCMerkleBlockEntity *e in [BTCMerkleBlockEntity allObjects]) {
            @autoreleasepool {
                BTCMerkleBlock *b = e.merkleBlock;
...
- (NSArray *)blockLocatorArray
{
    ...
    BTCMerkleBlock *b = self.lastBlock;
...
- (BTCMerkleBlock *)lastBlock
{
    if (! _lastBlock) {
        NSFetchRequest *req = [BTCMerkleBlockEntity fetchRequest];
        ...
        req.fetchLimit = 1;
        _lastBlock = [[BTCMerkleBlockEntity fetchObjects:req].lastObject merkleBlock];
        ...
        for (int i = CHECKPOINT_COUNT - 1; ! _lastBlock && i >= 0; i--) {
            if (i == 0 || checkpoint_array[i].timestamp + 7*24*60*60 < self.earliestKeyTime + NSTimeIntervalSince1970) {
                ...
                _lastBlock = [[BTCMerkleBlock alloc] initWithBlockHash:hash version:1 prevBlock:UINT256_ZERO
                              merkleRoot:UINT256_ZERO timestamp:checkpoint_array[i].timestamp
...
- (double)syncProgress
{
    ...
    if (self.downloadPeer.status != BTCPeerStatusConnected) return 0.05;
...
- (NSUInteger)peerCount
{
    ...
    for (BTCPeer *peer in [self.connectedPeers copy]) {
        if (peer.status == BTCPeerStatusConnected) count++;
    }
...
- (BTCBloomFilter *)bloomFilterForPeer:(BTCPeer *)peer
{
    ...
    for (BTCTransaction *tx in manager.wallet.allTransactions) { // find TXOs spent within the last 100 blocks
        ...
        for (NSValue *hash in tx.inputHashes) {
            ...
            BTCTransaction *t = [manager.wallet transactionForHash:o.hash];
    ...
    BTCBloomFilter *filter = [[BTCBloomFilter alloc] initWithFalsePositiveRate:self.fpRate
...
- (void)connect
{
    dispatch_async(self.q, ^{
        if ([BTCWalletManager sharedInstance].noWallet) return; // check to make sure the wallet has been created
        ...
        if (self.syncProgress < 1.0) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:BTCPeerManagerSyncStartedNotification
                 object:nil];
            });
        }

        [self.connectedPeers minusSet:[self.connectedPeers objectsPassingTest:^BOOL(id obj, BOOL *stop) {
            return ([obj status] == BTCPeerStatusDisconnected) ? YES : NO;
        }]];
        ...
        while (peers.count > 0 && self.connectedPeers.count < PEER_MAX_CONNECTIONS) {
            ...
            BTCPeer *p = peers[(NSUInteger)(pow(arc4random_uniform((uint32_t)peers.count), 2)/peers.count)];
        ...
        if (self.connectedPeers.count == 0) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                NSError *error = [NSError errorWithDomain:@"BTCWallet" code:1
                                  userInfo:@{NSLocalizedDescriptionKey:NSLocalizedString(@"no peers found", nil)}];

                [[NSNotificationCenter defaultCenter] postNotificationName:BTCPeerManagerSyncFailedNotification
...
- (void)addTransactionToPublishList:(BTCTransaction *)transaction
{
    NSLog(@"[BTCPeerManager] add transaction to publish list %@", transaction);
    if (transaction.blockHeight == TX_UNCONFIRMED) {
        ...
        for (NSValue *hash in transaction.inputHashes) {
            ...
            [hash getValue:&h];
            [self addTransactionToPublishList:[[BTCWalletManager sharedInstance].wallet transactionForHash:h]];
        }
    }
}

- (void)publishTransaction:(BTCTransaction *)transaction completion:(void (^)(NSError *error))completion
{
    NSLog(@"[BTCPeerManager] publish transaction %@", transaction);
    if (! transaction.isSigned) {
        if (completion) {
            completion([NSError errorWithDomain:@"BTCWallet" code:401 userInfo:@{NSLocalizedDescriptionKey:
        ...
        if (completion) {
            completion([NSError errorWithDomain:@"BTCWallet" code:-1009 userInfo:@{NSLocalizedDescriptionKey:
...
- (NSTimeInterval)timestampForBlockHeight:(uint32_t)blockHeight
{
    ...
    if (_blocks.count > 0) {
        if (blockHeight >= self.lastBlockHeight - BLOCK_DIFFICULTY_INTERVAL*2) { // recent block we have the header for
            BTCMerkleBlock *block = self.lastBlock;
    ...
    else [[BTCMerkleBlockEntity context] performBlock:^{ [self blocks]; }];
...
{
    [[BTCWalletManager sharedInstance].wallet setBlockHeight:height andTimestamp:timestamp forTxHashes:txHashes];
...
- (void)txTimeout:(NSValue *)txHash
{
    ...
    if (callback) {
        callback([NSError errorWithDomain:@"BTCWallet" code:BITCOIN_TIMEOUT_CODE userInfo:@{NSLocalizedDescriptionKey:
...
- (void)loadMempools
{
    ...
    for (BTCPeer *p in self.connectedPeers) { // after syncing, load filters and get mempools from other peers
                    ...
                    if (p == self.downloadPeer) {
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            [[NSNotificationCenter defaultCenter]
                             postNotificationName:BTCPeerManagerSyncFinishedNotification object:nil];
                        });
                    }
                }];
            }
...
- (void)removeUnrelayedTransactions
{
    ...
    for (BTCPeer *p in self.connectedPeers) { // don't remove tx until all peers have finished relaying their mempools
        if (! p.synced) return;
    }
...
- (void)updateFilter
{
    ...
    [self.downloadPeer sendPingMessageWithPongHandler:^(BOOL success) { // wait for pong so we include already sent tx
        ...
        if (self.lastBlockHeight < self.estimatedBlockHeight) { // if we're syncing, only update download peer
            ...
            for (BTCPeer *p in self.connectedPeers) {
                [p sendFilterloadMessage:[self bloomFilterForPeer:p].data];
...
- (void)peerMisbehavin:(BTCPeer *)peer
{
    peer.misbehavin++;
    ...
    if (++self.misbehavinCount >= 10) { // clear out stored peers so we get a fresh list from DNS for next connect
        ...
        [BTCPeerEntity deleteObjects:[BTCPeerEntity allObjects]];
        _peers = nil;
    }
...
- (void)sortPeers
{
    [_peers sortUsingComparator:^NSComparisonResult(BTCPeer *p1, BTCPeer *p2) {
...
- (void)savePeers
{
    NSLog(@"[BTCPeerManager] save peers");
    ...
    [[BTCPeerEntity context] performBlock:^{
        [BTCPeerEntity deleteObjects:[BTCPeerEntity objectsMatching:@"! (address in %@)", addrs]]; // remove deleted peers

        for (BTCPeerEntity *e in [BTCPeerEntity objectsMatching:@"address in %@", addrs]) { // update existing peers
            @autoreleasepool {
                BTCPeer *p = [peers member:[e peer]];
        ...
        for (BTCPeer *p in peers) {
            @autoreleasepool {
                [[BTCPeerEntity managedObject] setAttributesFromPeer:p]; // add new peers
            }
        }
    }];
}
...
- (void)saveBlocks
{
    NSLog(@"[BTCPeerManager] save blocks");
    ...
    [[BTCMerkleBlockEntity context] performBlock:^{
        [BTCMerkleBlockEntity deleteObjects:[BTCMerkleBlockEntity objectsMatching:@"! (blockHash in %@)",
                                            blocks.allKeys]];

        for (BTCMerkleBlockEntity *e in [BTCMerkleBlockEntity objectsMatching:@"blockHash in %@", blocks.allKeys]) {
            @autoreleasepool {
        ...
        for (BTCMerkleBlock *b in blocks.allValues) {
            @autoreleasepool {
                [[BTCMerkleBlockEntity managedObject] setAttributesFromBlock:b];
            }
        }
...
- (void)peerConnected:(BTCPeer *)peer
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [[NSNotificationCenter defaultCenter] postNotificationName:BTCPeerManagerTxStatusNotification object:nil];
    });
...
- (void)peer:(BTCPeer *)peer disconnectedWithError:(NSError *)error
{
    ...
    if ([error.domain isEqual:@"BTCWallet"] && error.code != BITCOIN_TIMEOUT_CODE) {
...
- (void)peer:(BTCPeer *)peer relayedPeers:(NSArray *)peers
{
    ...
    while (self.peers.count > 1000 && ((BTCPeer *)self.peers.lastObject).timestamp + 3*60*60 < now) {
...
- (void)peer:(BTCPeer *)peer hasTransaction:(UInt256)txHash
{
    ...
    BOOL syncing = (self.lastBlockHeight < self.estimatedBlockHeight);
    BTCTransaction *tx = self.publishedTx[hash];
    void (^callback)(NSError *error) = self.publishedCallback[hash];
...
{
    BTCWalletManager *manager = [BTCWalletManager sharedInstance];
    BTCTransaction *tx = [manager.wallet transactionForHash:txHash];
    NSValue *hash = uint256_obj(txHash);
...
- (void)peer:(BTCPeer *)peer relayedBlock:(BTCMerkleBlock *)block
{
    ...
    if (peer == self.downloadPeer && block.totalTransactions > 0) {
        ...
        if (self.downloadPeer.status == BTCPeerStatusConnected && self.fpRate > BLOOM_DEFAULT_FALSEPOSITIVE_RATE*10.0) {
    ...
    BTCMerkleBlock *prev = self.blocks[prevBlock];
    ...
    if ((block.height % BLOCK_DIFFICULTY_INTERVAL) == 0) { // hit a difficulty transition, find previous transition time
        BTCMerkleBlock *b = block;

        for (uint32_t i = 0; b && i < BLOCK_DIFFICULTY_INTERVAL; i++) {
            b = self.blocks[uint256_obj(b.prevBlock)];
        }

        [[BTCMerkleBlockEntity context] performBlock:^{ // save transition blocks to core data immediately
            @autoreleasepool {
                BTCMerkleBlockEntity *e = [BTCMerkleBlockEntity objectsMatching:@"blockHash == %@",
                                          [NSData dataWithBytes:b.blockHash.u8 length:sizeof(UInt256)]].lastObject;
        
                if (! e) e = [BTCMerkleBlockEntity managedObject];
                [e setAttributesFromBlock:b];
            }
            
            [BTCMerkleBlockEntity saveContext]; // persist core data to disk
        }];
    ...
    if (uint256_eq(block.prevBlock, self.lastBlock.blockHash)) { // new block extends main chain
        ...
        BTCMerkleBlock *b = block, *b2 = self.lastBlock;
        ...
        for (BTCTransaction *tx in [BTCWalletManager sharedInstance].wallet.allTransactions) {
        ...
        while (b.height > b2.height) { // set transaction heights for new main chain
            ...
            b = self.blocks[uint256_obj(b.prevBlock)];
            txTime = b.timestamp/2 + ((BTCMerkleBlock *)self.blocks[uint256_obj(b.prevBlock)]).timestamp/2;
        }
    ...
    if (block == self.lastBlock && self.orphans[blockHash]) {
        BTCMerkleBlock *b = self.orphans[blockHash];
...
- (void)peer:(BTCPeer *)peer setFeePerKb:(uint64_t)feePerKb
{
    ...
    for (BTCPeer *p in self.connectedPeers) { // find second highest fee rate
...
- (BTCTransaction *)peer:(BTCPeer *)peer requestedTransaction:(UInt256)txHash
{
    ...
    if (callback && ! [manager.wallet transactionIsValid:tx]) {
        ...
        error = [NSError errorWithDomain:@"BTCWallet" code:401
        ...
        [[BTCTransactionEntity context] performBlock:^{
            [BTCTransactionEntity saveContext]; // persist transactions to core data
        }];
    }
